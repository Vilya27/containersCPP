...
// для сета нужен find
template <typename T>
typename list<T>::const_iterator list<T>::find(const_reference value) const {
...
// Перегружаем на константу, как в std::list
...
void list<value_type>::unique() {
  for (iterator it = this->begin(); it != this->end();) {
    if (it.ptr_ != nullptr && it.ptr_->prev != nullptr &&
        it.ptr_->data == it.ptr_->prev->data) {
      iterator del_it = it;
      --it;
      this->erase(del_it);
    } else {
