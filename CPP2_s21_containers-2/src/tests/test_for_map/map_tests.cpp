...
TEST(MapTest, BeginEndIterators) {
  s21::map<const int, int> map{{1, 2}, {3, 4}, {5, 6}};
  std::map<const int, int> std_map{{1, 2}, {3, 4}, {5, 6}};
  auto begin_it = map.begin();
  auto end_it = map.end();
  auto begin_it_std = std_map.begin();
  EXPECT_EQ(*begin_it, *begin_it_std);
  ++begin_it;
  ++begin_it_std;
  EXPECT_EQ(*begin_it, *begin_it_std);
  ++begin_it;
...
TEST(MapTest, ConstBeginEndIterators) {
  s21::map<const int, int> map{{1, 2}, {3, 4}, {5, 6}};
  std::map<const int, int> std_map{{1, 2}, {3, 4}, {5, 6}};
  s21::map<const int, int>::const_iterator begin_it = map.begin();
  s21::map<const int, int>::const_iterator end_it = map.end();
  std::map<const int, int>::const_iterator begin_it_std = std_map.begin();
  EXPECT_EQ(*begin_it, *begin_it_std);
  ++begin_it;
  ++begin_it_std;
  EXPECT_EQ(*begin_it, *begin_it_std);
...
TEST(MapTest, MaxSize) {
  ...
  EXPECT_EQ(my_map.max_size(), my_list.max_size());
}
